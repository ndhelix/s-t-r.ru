...
namespace NopSolutions.NopCommerce.BusinessLogic.Categories
{
    ...
    public partial class CategoryManager
    {
        ...
        private const string CATEGORIES_EVERYTHING_KEY = "Nop.category.everything";
        ...
        public static CategoryCollection GetEverythingCategories()
        {
          string key = string.Format(CATEGORIES_EVERYTHING_KEY);
          object obj2 = NopCache.Get(key);
          if (CategoryManager.CategoriesCacheEnabled && (obj2 != null))
          {
            return (CategoryCollection)obj2;
          }
          var dbCollection = DBProviderManager<DBCategoryProvider>.Provider.GetEverythingCategories();
          var categoryCollection = DBMapping(dbCollection);
